...
def rfiCurtain(img_name, op_path, size_x, size_y, binary):
	...
	if rfi_axis == 'x':
		rfi_start = random.randint(size_x/20,size_x - (size_x/20))
		...
		rfi_start = random.randint(size_y/20,size_y - (size_y/20))
		rfi_width = int(size_y/(10*random.randint(1,size_y)))
	rfi_width = random.randint(1,3)	#if random.randint(2,4)>2 else 2
...
def pulsarCurtain(img_name, op_path, size_x, size_y, binary):
	rfi_start_x = random.randint(1,size_x/2-1)
	rfi_end_x =  random.randint(1+rfi_start_x + size_x/2 -1,size_x)
	rfi_start_y = random.randint(1,size_y/2-1)
	rfi_end_y = random.randint(1+rfi_start_y+size_y/2 -1,size_y)
	rfi_width = 1#random.randint(2,3)
	img = [0 for j in range(size_x*size_y)]
...
def generateRFIData(img_name, op_path, size_x, size_y, binary):
	#if(random.random()>0.5):
	return rfiCurtain(img_name, op_path, size_x, size_y, binary)
	#else:
	#	return pulsarCurtain(img_name, op_path, size_x, size_y, binary)
def generatePulsarData(img_name, op_path, size_x, size_y, binary):
	#if(random.random()>0.5):
	return pulsarCurtain(img_name, op_path, size_x, size_y, binary)
	#else:
	#	
...
if __name__ == "__main__":
	...
	for imgcnt in range(num_images):
		...
		samples.append(generateRFIData(img_name, op_path, size_x, size_y, binary))
		samples.append(generatePulsarData(img_name, op_path, size_x, size_y, binary))
