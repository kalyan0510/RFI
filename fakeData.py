...
# python fakeData.py img/ 20 20 0 100
...
def pulsarCurtain(img_name, op_path, size_x, size_y, binary):
	...
	rfi_end_x =  random.randint(10+rfi_start_x,size_x)
	...
	return (img, np.int64(2),np.int64(rfi_start_x),np.int64(rfi_width))
...
def generatePandomData(img_name, op_path, size_x, size_y, binary):
	#if(random.random()>0.5):
	return pulsarCurtain(img_name, op_path, size_x, size_y, binary)
	#else:
	#	
...
if __name__ == "__main__":
	...
	for imgcnt in range(num_images):
		...
		#samples.append(generatePandomData(img_name, op_path, size_x, size_y, binary))
